...
class _SeasonDetailScreenState extends ConsumerState<SeasonDetailScreen> {
  ...
  Future<void> _loadTasks() async {
    ...
    try {
      ...
      final tasks = await getDailyTasks.execute(widget.season.id, DateTime.now());
      
      setState(() {
        _tasks = tasks;
        _currentDay = null; // TODO: Need API to provide this
        _currentStage = null; // TODO: Need API to provide this
        _isLoading = false;
      });
    } catch (e) {
      setState(() {
  ...
  Future<void> _handleTaskSkip(TaskEntity task) async {
    // Bỏ qua = Ẩn task vĩnh viễn khỏi danh sách
    print('Bỏ qua task: ${task.taskName}');
    
    final taskService = ref.read(taskServiceProvider);
    final success = await taskService.hideTask(
      seasonId: widget.season.id,
  ...
  String _getTaskDateRange(TaskEntity task) {
  ...
  Future<void> _confirmDeleteSeason() async {
    ...
    if (confirm == true && mounted) {
      try {
        ...
        await seasonService.deleteSeason(widget.season.id);
        if (mounted) {
